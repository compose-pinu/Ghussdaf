// set bash title
process.stdout.write("\x1b]2;SIDDIK-BOT - Made by SK SIDDIK\x1b\x5c");
...
const titles = [
	...
	[
		"S I D D I K B O T  V 2 @" + currentVersion
	],
	[
		"SIDDIKBOT V2"
	]
...
async function startBot(loginWithEmail) {
	...
	const tooOldVersion = (await axios.get("https://raw.githubusercontent.com/DJ-SIDDIK-15/Notific/refs/heads/main/tooOldVersions.txt")).data || "0.0.0";
	// nếu version cũ hơn
	...
	log.info("SIDDIK FACEBOOK", getText('login', 'currentlyLogged'));
	...
	(function loginBot(appState) {
		...
		login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
				...
				if (facebookAccount.email && facebookAccount.password) {
					...
					setTimeout(async function refreshCookie() {
						try {
							...
							log.err("SIDDIK REFRESH COOKIE", getText('login', 'refreshCookieError'), err.message, err);
			...
			log.info("SK SIDDIK NODE VERSION", process.version);
			log.info("SK SIDDIK PROJECT VERSION", currentVersion);
			log.info("SK SIDDIK BOT ID", `${global.botID} - ${await getName(global.botID)}`);
			log.info("SK SIDDIK PREFIX", global.GoatBot.config.prefix);
			log.info("SK SIDDIK LANGUAGE", global.GoatBot.config.language);
			log.info("SK SIDDIK BOT NICK NAME", global.GoatBot.config.nickNameBot || "GOAT BOT");
			// ———————————————————— GBAN ————————————————————— //
			let dataGban;

			try {
				// convert to promise
				const item = await axios.get("https://raw.githubusercontent.com/DJ-SIDDIK-15/Notific/refs/heads/main/gban.json");
				dataGban = item.data;
			...
			let notification;
			try {
				const getNoti = await axios.get("https://raw.githubusercontent.com/DJ-SIDDIK-15/Notific/refs/heads/main/notification.txt");
			...
			log.master("SK SIDDIK NOTIFICATION", (notification || "").trim());
			log.master("SK SIDDIK SUCCESS", getText('login', 'runBot'));
			log.master("SK SIDDIK LOAD TIME", `${convertTime(Date.now() - global.GoatBot.startTime)}`);
			logColor("#f5ab00", createLine("COPYRIGHT"));
			// —————————————————— COPYRIGHT INFO —————————————————— //
			// console.log(`\x1b[1m\x1b[33mCOPYRIGHT:\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36mProject SIDDIK BOT created by SK SIDDIK (https://github.com/SK-SIDDIK), please do not sell this source code or claim it as your own. Thank you!\x1b[0m`);
			...
			if (global.GoatBot.config.serverUptime.enable == true && !global.GoatBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
				...
				const { data: html } = await axios.get("https://raw.githubusercontent.com/DJ-SIDDIK-15/Notific/refs/heads/main/home.html");
